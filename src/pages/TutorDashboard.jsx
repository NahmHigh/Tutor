...
import { Container, Row, Col, Card, Button, Table, Badge, Alert, Tabs, Tab, Form, Modal } from 'react-bootstrap';
import { useAuth } from '../auth/AuthContext';
import { useAppContext } from '../provider/AppProvider';
import { instance } from '../lib/axios';
import {
    FaCalendarAlt, FaUsers, FaStar, FaClock,
    FaCheckCircle, FaTimesCircle, FaEdit, FaEye,
    FaPlus
} from 'react-icons/fa';
...
const TutorDashboard = () => {
    ...
    const currentTutor = useMemo(() => {
        return tutors.find(tutor => tutor.userId?.toString() === user?.id?.toString()) ||
    ...
    const tutorSchedules = useMemo(() => {
        ...
        return schedules.filter(schedule =>
            schedule.tutorId?.toString() === currentTutor.id?.toString()
        );
    ...
    const tutorReviews = useMemo(() => {
        ...
        return reviews.filter(review =>
            review.tutorId?.toString() === currentTutor.id?.toString()
        );
    ...
    const stats = useMemo(() => {
        ...
        const completedSchedules = tutorSchedules.filter(s => s.status === 'completed').length;
        const pendingSchedules = tutorSchedules.filter(s => s.status === 'pending').length;
        const totalStudents = new Set(tutorSchedules.map(s => s.studentId)).size;
        const totalEarnings = tutorSchedules
            .filter(s => s.status === 'completed')
            .reduce((sum, schedule) => sum + (currentTutor?.hourlyRate || 0) * (schedule.duration / 60), 0);
        ...
        return {
            ...
            totalStudents,
            totalEarnings,
            averageRating: currentTutor?.rating || 0,
    ...
    const getStudentName = (studentId) => {
        const student = users.find(u => u.id?.toString() === studentId?.toString());
                        ...
                        <Col md={3} className="mb-3">
                            <Card className="text-center border-info">
                                <Card.Body>
                                    ...
                                    <h4 className="fw-bold">{stats.totalEarnings.toLocaleString('vi-VN')}</h4>
                                                ...
                                                <tr key={schedule.id}>
                                                    ...
                                                    <td>
                                                        ...
                                                        <Button
                                                            variant="danger"
                                                            size="sm"
                                                            onClick={() => handleScheduleAction(schedule.id, 'cancelled')}
                                                        >
                ...
                <Tab eventKey="reviews" title="Đánh giá">
                    <Card>
                        <Card.Header className="d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <span className="text-warning h4">{stats.averageRating}/5</span>
                                <small className="text-muted"> ({stats.totalReviews} đánh giá)</small>
                            </div>
                        </Card.Header>
                        <Card.Body>
                    ...
                    {selectedSchedule && (
                        <Row>
                            <Col md={6}>
                                ...
                                <p><strong>Thời gian:</strong> {formatTime(selectedSchedule.startTime)} - {formatTime(selectedSchedule.endTime)}</p>
                                <p><strong>Thời lượng:</strong> {selectedSchedule.duration} phút</p>
                                <p><strong>Học sinh:</strong> {getStudentName(selectedSchedule.studentId)}</p>
                            </Col>
                            <Col md={6}>
                                <p><strong>Môn học:</strong> {selectedSchedule.subject}</p>
                                <p><strong>Địa điểm:</strong> {selectedSchedule.location}</p>
                                <p><strong>Trạng thái:</strong> {getStatusBadge(selectedSchedule.status)}</p>
                                <p><strong>Ghi chú:</strong> {selectedSchedule.notes || 'Không có'}</p>
                            </Col>
                        </Row>
                    )}
                </Modal.Body>
